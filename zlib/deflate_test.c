...
int test_zlib(void) {
	...
	unsigned char* zlib_compressed_data = zlib_deflate((unsigned char*) zlib_deflate_test, sizeof(zlib_data), &zlib_compressed_data_length, NULL, &err);
	if (err) {
...
int main(void) {
	...
	printf("Testing full ZLIB\n");
	if (test_zlib()) return -1;
